import { editArchiveFileWithImgServer } from '@/entities/archive/file/api/file.server'
...
export const PATCH = async (request: Request) => {
  ...
  try {
    ...
    if (!dataSourceId) throw new Error('dataSourceId 누락')
    ...
    const response = await requireAuth(async token =>
      editArchiveFileWithImgServer(dataSourceId, backendFormData, { token })
    )
